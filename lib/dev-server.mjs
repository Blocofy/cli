 ...
 */
export function startDevServer({ dir, url, token, port, syncDraft = false, onError, onReload, onRetry }) {
  ...
  const watcher = startWatch(dir, async (file) => {
    ...
    if (syncDraft) {
      try {
        await pushTheme({ dir, url, token, draft: true, onRetry });
      } catch (err) {
  ...
  if (syncDraft) {
    void pushTheme({ dir, url, token, draft: true, onRetry }).catch(() => {});
  }
