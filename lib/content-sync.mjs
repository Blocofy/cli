 ...
 */
export async function probePagesServer({ url, token }) {
  const res = await fetch(`${baseOf(url)}/api/dev/content?scope=capabilities`, { headers: { authorization: `Bearer ${token}` } });
  ...
  return { defaultLocale: body.default_locale, supportedLocales: body.supported_locales ?? [] };
}

/** Site içeriğini diske çek (pull). Pages require protocol v2; every path is contained and staged. */
export async function pullContent({ dir, url, token, scope, writeFile }) {
  const res = await fetch(`${baseOf(url)}/api/dev/content?scope=${encodeURIComponent(scope)}`, {
    headers: { authorization: `Bearer ${token}` },
  });
  const body = await res.json().catch(() => null);
  if (!res.ok) {
    throw new PagesCliError(body?.code ?? "PAGES_PULL_FAILED", body?.error ?? `Pull failed (HTTP ${res.status}); nothing was written.`, { status: res.status });
  }
 ...
 */
export async function pushContent({ dir, url, token, scope, dryRun = false }) {
  ...
  const server = await probePagesServer({ url, token });
  ...
  const res = await fetch(`${base}/api/dev/content`, {
    method: "POST",
    headers: { "content-type": "application/json", authorization: `Bearer ${token}` },
    body: JSON.stringify({ protocol_version: PAGES_PROTOCOL_VERSION, dry_run: dryRun, files }),
  });
  const body = await res.json().catch(() => null);
  if (!res.ok || body?.ok !== true) {
    throw new PagesCliError(body?.code ?? "PAGES_PUSH_FAILED", body?.error ?? `Push failed (HTTP ${res.status}).`, {
      diagnostics: body?.diagnostics ?? [],
      pages: body?.pages ?? null,
      status: res.status,
    });
  }
  return { ...body, fileCount: local.length };
}
 ...
 */
export async function checkPages({ dir, url, token }) {
  const { files: local, errors } = collectPageFiles(dir);
  const diagnostics = [...errors];
  let server = null;
  if (url && token) server = await probePagesServer({ url, token });
  diagnostics.push(...checkLocalBatch(local.map(([p, c]) => classifyPageFile(p, c, server ?? {}))));

  let plan = null;
  if (server && !diagnostics.some((d) => d.level === "error")) {
    try {
      plan = await pushContent({ dir, url, token, scope: "pages", dryRun: true });
 ...
 */
export async function migrateLayout({ dir, url, token, write = false }) {
