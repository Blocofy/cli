...
const V2_KEY_ORDER = ["format_version", "base_revision", "slug", "title", "status", "seo_title", "seo_description", "og_image", "canonical_url", "locale", "template", "data"];
...
/** CF-T3 — the fingerprint `blocofy pages pull` stamps (mirror of the platform's `page-files/base-revision.ts`). */
export const BASE_REVISION_PATTERN = /^r1_[0-9a-f]{32}$/;
...
export function classifyPageFile(path, content, ctx = {}) {
  ...
  // CF-T3 — absent/null is judged by the push (it needs the server state); anything else must be a pulled value.
  if (json.base_revision != null && (typeof json.base_revision !== "string" || !BASE_REVISION_PATTERN.test(json.base_revision))) {
    return fail("PAGES_INVALID_JSON", "base_revision is not a value `blocofy pages pull` wrote", { slug });
  }
...
export function formatDiagnostic(d) {
  ...
  // CF-T3 — a stale or base-less file shows what it was pulled at and what the page is now.
  const revs = d.current_revision ? `\n    base_revision ${d.base_revision ?? "(none)"} → current ${d.current_revision}` : "";
  return `${d.level} [${d.code}]:\n    ${d.message}${where}${revs}`;
}
