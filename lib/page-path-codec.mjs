...
// COM0/LPT0 included: Microsoft lists them as reserved too.
...
export function encodeSegment(segment) {
  ...
  // A segment spelled `index.json` would make a route's own file name (`…/<seg>/index.json`) the folder of a
  // child route (`/blog` vs `/blog/index.json`), which no filesystem or git tree can hold. Escaping its first
  // byte keeps the two apart.
  const reserved = WINDOWS_RESERVED.test(segment.split(".")[0] ?? "") || segment === "index.json";
  let out = "";
