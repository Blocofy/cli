...
/** Download one exported asset's bytes from its own delivery URL (not the v1 API — no auth header). */
export async function downloadAssetBytes({ url: assetUrl, onRetry = null }) {
  const res = await fetchWithRetry(assetUrl, {}, { onRetry });
  if (!res.ok) throw new Error(`could not download ${assetUrl} (HTTP ${res.status})`);
