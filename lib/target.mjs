...
function contextMatchesBinding(ctx, project) {
  return isObj(ctx?.site) && sameId(ctx.site.id, project.site_id) && origin(ctx.platform_origin) === origin(project.platform_origin);
}
 ...
 * binding. Returns the verified identity (`site`, `platformOrigin`, `liveThemeId`).
 */
export async function verifyTarget({ resolved, secrets, binding, fetchImpl = fetch }) {
  ...
  if (isObj(ctx.site) && (!sameId(ctx.site.id, identity.site.id) || origin(ctx.platform_origin) !== identity.platformOrigin)) {
    throw new TargetError(
      "TARGET_SITE_MISMATCH",
  ...
  if (binding && (!sameId(binding.project.site_id, identity.site.id) || origin(binding.project.platform_origin) !== identity.platformOrigin)) {
    throw new TargetError(
      "TARGET_SITE_MISMATCH",
