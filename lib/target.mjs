...
import { existsSync, linkSync, lstatSync, mkdirSync, readFileSync, readdirSync, renameSync, rmSync, rmdirSync, statSync, writeFileSync } from "node:fs";
...
/**
 * Write `.blocofy/project.json` (+ `local.json` for a named context, + `.gitignore`). Review I2: never follows a
 * symlink — `.blocofy` must be a real directory and none of the three files may be a symlink (checked for all
 * before the first write); each file is written to a fresh temp file in the same directory (`wx`, 0644) and
 * renamed into place. `staleLocal: true` (link) removes a `local.json` that an env-context binding does not own.
 */
export function writeBinding(root, { site, platformOrigin, contextName, staleLocal = false }) {
  const d = join(root, PROJECT_DIR);
  const refuse = (path, what) => {
    throw new TargetError("TARGET_BINDING_INVALID", `${path} ${what}; the project binding was not written.`, { binding: path });
  };
  let st = lstatOrNull(d);
  if (st && (st.isSymbolicLink() || !st.isDirectory())) refuse(d, "is not a real directory (symlink or file)");
  if (!st) {
    mkdirSync(root, { recursive: true });
    mkdirSync(d);
    st = lstatSync(d);
    if (st.isSymbolicLink() || !st.isDirectory()) refuse(d, "is not a real directory");
  }
  const project = { schema_version: 1, site_id: site.id, site_slug: site.slug ?? null, platform_origin: origin(platformOrigin) };
  const writes = [["project.json", JSON.stringify(project, null, 2) + "\n"]];
  const ownsLocal = contextName && contextName !== ENV_CONTEXT;
  if (ownsLocal) writes.push(["local.json", JSON.stringify({ context: contextName }, null, 2) + "\n"]);
  writes.push([".gitignore", "local.json\n"]);
  for (const name of ["project.json", "local.json", ".gitignore"]) {
    const f = lstatOrNull(join(d, name));
    if (f && (f.isSymbolicLink() || !f.isFile())) refuse(join(d, name), "is a symlink or not a regular file");
  }
  for (const [name, content] of writes) {
    const target = join(d, name);
    const tmp = join(d, `.${name}.${process.pid}.${randomBytes(6).toString("hex")}.tmp`);
    try {
  ...
  if (!ownsLocal && staleLocal) rmSync(join(d, "local.json"), { force: true });
  return join(d, "project.json");
}

/**
 * Review M4 — a pull into a new directory claims `.blocofy/project.json` EXCLUSIVELY before writing content, so two
 * concurrent pulls into the same empty directory cannot both write. The complete file is hard-linked into place
 * (fails atomically with EEXIST; a reader never sees a half-written file). If another process claimed it first,
 * its binding must name the same site (else TARGET_SITE_MISMATCH, nothing written by us). Returns `{ release }`:
 * undoes our claim (and the directories it created, if still empty) when the pull fails.
 */
export function claimNewBinding(root, { site, platformOrigin }) {
  const createdRoot = !existsSync(root);
  mkdirSync(root, { recursive: true });
  const d = join(root, PROJECT_DIR);
  const st = lstatOrNull(d);
  if (st && (st.isSymbolicLink() || !st.isDirectory())) {
    throw new TargetError("TARGET_BINDING_INVALID", `${d} is not a real directory; nothing was written.`, { binding: d });
  }
  ...
  const projectPath = join(d, "project.json");
  const project = { schema_version: 1, site_id: site.id, site_slug: site.slug ?? null, platform_origin: origin(platformOrigin) };
  const tmp = join(d, `.project.json.${process.pid}.${randomBytes(6).toString("hex")}.tmp`);
  writeFileSync(tmp, JSON.stringify(project, null, 2) + "\n", { flag: "wx", mode: 0o644 });
  try {
    linkSync(tmp, projectPath);
  } catch (error) {
    if (error?.code !== "EEXIST") throw error;
    const other = readBinding(root);
    const o = compareOrigin(other.project.platform_origin, platformOrigin);
    if (!sameId(other.project.site_id, site.id) || o === "mismatch" || o === "unproven") {
      throw new TargetError(
        "TARGET_SITE_MISMATCH",
        `${root} was just bound to site ${other.project.site_slug ?? other.project.site_id} by another command; this pull is for ${site.slug ?? site.id}. Nothing was written.`,
        { dir: root, binding_site_id: other.project.site_id, remote_site_id: site.id },
      );
    }
  ...
  const tryRmdir = (p) => {
    try {
      ...
      /* not empty (someone else wrote) — leave it */
    }
  };
  return {
    claimed: true,
    release() {
      ...
      if (createdDir) tryRmdir(d);
      if (createdRoot) tryRmdir(root);
    },
  };
}
...
/** Identity reads use the shared retry contract (lib/http.mjs); `retry` = `{ onRetry, sleep, now }` overrides. */
async function getJson(fetchImpl, url, secret, retry = {}) {
  let res;
  try {
    res = await fetchWithRetry(url, { headers: { authorization: `Bearer ${secret}`, accept: "application/json" } }, { ...retry, fetchImpl, timeoutMs: IDENTITY_TIMEOUT_MS });
  } catch (error) {
...
export async function verifyDev({ url, token, fetchImpl = globalThis.fetch, retry = {} }) {
  const endpoint = `${url.replace(/\/+$/, "")}/api/dev/whoami`;
  const r = await getJson(fetchImpl, endpoint, token, retry);
...
export async function verifyApi({ url, apiKey, fetchImpl = globalThis.fetch, retry = {} }) {
  const endpoint = `${url.replace(/\/+$/, "")}/api/v1/ping`;
  const r = await getJson(fetchImpl, endpoint, apiKey, retry);
 ...
 */
export async function verifyTarget({ resolved, secrets, binding, fetchImpl = globalThis.fetch, retry = {} }) {
  const ctx = resolved.context;
  const dev = ctx.dev && secrets.devToken ? await verifyDev({ url: ctx.dev.url, token: secrets.devToken, fetchImpl, retry }) : null;
  const api = ctx.api && secrets.apiKey ? await verifyApi({ url: ctx.api.url, apiKey: secrets.apiKey, fetchImpl, retry }) : null;
