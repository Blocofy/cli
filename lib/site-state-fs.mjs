...
/** sha256 (lowercase hex) of a `Buffer`. */
...
/**
 * Staged write of a site-state tree. `entries` is `[relPath, content]`, content a string or a `Buffer`
 * (asset bytes). Every path is validated (traversal/absolute/backslash/symlink-in-ancestor) before the
 * first byte is written; then every file is staged into a temp directory under `root`, and only when
 * every file is staged are they renamed into place. Mirrors `page-files.mjs`'s `stagedWrite`,
 * generalized to binary content.
 *
 * WHAT THIS DOES AND DOES NOT GUARANTEE. A failure anywhere BEFORE the rename phase - a bad path, a
 * symlinked ancestor, a full disk while staging - leaves the destination exactly as it was. The rename
 * phase itself is not atomic across files: renames land one at a time, across directories, and an I/O
 * failure part-way through leaves the ones already renamed in place and throws "Stopped after writing
 * N of M files". The caller is `site export`, a local read-then-write that is safe to re-run, so a
 * partial tree is recoverable by running it again. Do not reuse this for a remote mutation, or for
 * anything where a half-applied tree is not recoverable, without adding a real commit step.
 */
