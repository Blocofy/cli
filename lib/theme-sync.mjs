 ...
 */
/** Root names a developer's own tooling owns: a theme pull writes theme files, never these. */
const ROOT_FILE_DENY = new Set([
...
function pullPathProblem(rel) {
  ...
  if (segs.length === 1) {
    ...
    // `README.md`) and serves them on pull; refusing them refused the whole download. Dot-files stay out, and
    // so do the names a developer's own tooling owns — a pull must never overwrite those.
    if (segs[0].startsWith(".")) return "hidden file";
    return ROOT_FILE_DENY.has(segs[0].toLowerCase()) ? "a tooling file the pull must not overwrite" : null;
  }
