...
export async function fetchDevSession({ url, token, name = null }) {
  ...
  if (!res.ok) {
    if (res.status === 410) {
      // Sunucu bu ucu emekliye ayırdı (M1 preview-security). Uzak önizleme + editör görünümü yok;
      // taslak senkronu ve `theme publish` bu uca bağlı DEĞİL, ayrı yollardan çalışır.
      throw new Error(
        "this Blocofy server no longer offers CLI remote preview (HTTP 410). " +
          "Draft sync and `theme publish` are unaffected; the live-domain and editor views are not available.",
      );
    }
...
export async function pushTheme({ dir, url, token, draft = false, instance = null, name = null, dryRun = false, idempotencyKey = null, onRetry = null }) {
  ...
  if (idempotencyKey && !dryRun) {
    ...
    //     retained-rows kuralı korur. Taslak hedefte `?name=` taşınır: adsız probe taslağı
    //     `CLI Draft — <tarih>` olarak yaratır ve POST'un --name'i reuse yolunda yok sayılırdı.
    //  3) Gerçek POST. Dev-sync key göndermediği için bu aşamaların hiçbirini ödemez.
    const pre = await fetchWithRetry(
      `${base}/api/dev/theme`,
      { method: "POST", headers: headersFor({ "content-type": "application/json", "x-idempotency-key": idempotencyKey }), body: JSON.stringify({ ...(instance ? { files, instance } : { files, draft, ...(name ? { name } : {}) }), dryRun: true }) },
      { onRetry },
    );
    ...
    const query = instance
      ...
      : draft
        ? `?draft=1${name ? `&name=${encodeURIComponent(name)}` : ""}`
        : "";
    const probe = await fetch(`${base}/api/dev/theme${query}`, { headers: headersFor() });
