...
/**
 * Token'ın gerçek site'ını çözer (`GET /api/dev/whoami`). Site sunucu tarafında
 * TOKEN'dan çözülür — login URL'i kozmetik. CLI bunu `login`'de (doğrula+sakla)
 * ve `push` öncesi (hedef tenant'ı göster) çağırır; yanlış-tenant'a yazımı
 * görünür kılar. `{ site: { id, slug, name }, liveThemeId }`.
 */
