...
export async function pullTheme({ dir, url, token, draft = false, instance = null }) {
  ...
  // server path that escapes the directory or targets the CLI's own `.blocofy/` binding refuses the whole pull.
  const entries = Object.entries(files ?? {}).map(([key, content]) => [localPathFor(key), content]);
  const reserved = entries.filter(([rel]) => rel.split("/").some((seg) => seg === ".blocofy" || seg.startsWith(".blocofy-staging-")));
  if (reserved.length) {
    throw new PagesCliError("PAGES_PATH_ESCAPE", "The server sent a reserved path; nothing was written.", {
      diagnostics: reserved.map(([rel]) => ({ level: "error", code: "PAGES_PATH_ESCAPE", message: "reserved CLI path", path: rel })),
    });
  }
...
export async function pushTheme({ dir, url, token, draft = false, instance = null, name = null, dryRun = false, idempotencyKey = null, onRetry = null, prune = false, confirmPrune = null }) {
  ...
  if (idempotencyKey && !dryRun) {
    ...
    if (!instance && draft) {
      const status = await fetchSiteStatus({ url, token });
