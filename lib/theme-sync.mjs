 ...
 */
export async function pushTheme({ dir, url, token, draft = false, instance = null, name = null, onRetry = null }) {
  const base = url.replace(/\/+$/, "");
  const files = readLocalTemplates(dir);
  const payload = instance ? { files, instance } : name ? { files, draft, name } : { files, draft };
